"""Live spin gate for the G-Force Centrifuge Torture Test.

Boots BeamNG.drive in the sentinel-isolated profile with the packaged
centrifuge installed, spawns the prop plus a normal-sized ETK800 subject
inside the bowl, and proves the full torture chain:

- the drum arms on occupancy and climbs the RPM plateaus,
- the tangential velocity field drags the car into a real orbit against
  the banked wall (measured tangential speed and orbit radius),
- maximum RPM is held, then the sample is flung out over the rim,
- the drum spins down and the machine re-arms,
...
MOD_KEY = "gforce_centrifuge"
MOD_ID = "ericrolph_gforce_centrifuge"
ZIP_BASENAME = "gforce_centrifuge_ericrolph.zip"
RUNTIME_EXTENSION = "ericrolph__gforce__centrifuge_runtime"
LOG_TAG = "ERICROLPH_GFORCE_CENTRIFUGE_RUNTIME"
LIVE_TEST_TAG = "GFORCE_CENTRIFUGE_LIVE_TEST"
...
EXPECTED_TRIGGERS = {"drum_zone": "Overlaps", "rim_zone": "Overlaps"}
# Drop the subject at authored (3, 0) — off-centre so the tangential field
# grabs it immediately; identity spawn = 180-degree Z flip, so that is
# world (-3, 0) from the prop origin.
START_WORLD_OFFSET_X = -3.0
MIN_ORBIT_SPEED_MPS = 14.0
MIN_ORBIT_RADIUS = 4.0
MAX_ORBIT_RADIUS = 11.5
MIN_PEAK_TANGENTIAL = 22.0
...
def _configured_runtime() -> tuple[Path, Path, Path]:
    ...
    if not home_value or not user_value or not binary_value:
        pytest.skip(
            ...
            "BEAMNG_MCP_TEST_BEAMNG_BINARY for the G-Force Centrifuge live gate"
        )
    ...
    if not (user / ".beamng-mcp-test-user").is_file():
        pytest.fail("the G-Force Centrifuge live gate requires a sentinel-isolated profile")
...
def test_gforce_centrifuge_spins_and_flings_normal_car(tmp_path: Path) -> None:
    ...
    installed_zip = require_confined_profile_target(
        user, Path("mods") / f"gforce_live_{suffix}.zip"
    )
    scenario_name = f"gforce_live_{suffix}"
    ...
    log_start = f"gforce_live_start_{suffix}"
    ...
    with ExitStack() as safety:
        ...
        try:
            ...
            scenario = Scenario(
                "smallgrid",
                scenario_name,
                description="Disposable G-Force Centrifuge live spin fixture",
            )
            subject = Vehicle(SUBJECT_NAME, "etk800", license="SPIN ME")
            ...
            prop = Vehicle(PROP_NAME, MOD_ID, license="SPINNER")
            ...
            # Drop the sample into the bowl, off-centre.
            subject.teleport(
                pos=(origin_x + START_WORLD_OFFSET_X, origin_y, surface_z + 1.2),
                rot_quat=(0, 0, 0, 1),
                reset=True,
            )
            bng.control.step(30, wait=True)
            subject.control(parkingbrake=0.0, throttle=0.0, brake=0.0)

            armed_state: dict[str, Any] = {}
            for _ in range(30):
                bng.control.step(15, wait=True)
                armed_state = _runtime_state(bng)
                if armed_state.get("behavior_phase") == "spinning":
                    break
            assert armed_state.get("behavior_phase") == "spinning", armed_state
            ...
            for _ in range(160):
                ...
                if not probe.get("ok"):
                    continue
                radius = math.hypot(float(probe["x"]) - origin_x, float(probe["y"]) - origin_y)
                if (
                    float(probe["speed"]) > MIN_ORBIT_SPEED_MPS
            ...
            # Ride the plateaus to the fling; the FSM advances on timers.
            completed_state: dict[str, Any] = {}
            flung = False
            for _ in range(280):
                bng.control.step(15, wait=True)
                completed_state = _runtime_state(bng)
                stats = completed_state.get("behavior_stats") or {}
                if int(stats.get("flings", 0)) >= 1:
                    flung = True
                    break
            assert flung, {
                "detail": "centrifuge never flung the sample",
                "state": completed_state,
            }
            stats = completed_state.get("behavior_stats") or {}
            assert int(stats.get("max_rpm", 0)) >= 500, stats
            assert float(stats.get("peak_tangential_mps", 0.0)) >= MIN_PEAK_TANGENTIAL, stats

            # Spin-down back to idle once the bowl is clear.
            subject.teleport(
                pos=(origin_x + 40.0, origin_y + 40.0, surface_z + 0.5),
                rot_quat=(0, 0, 0, 1),
                reset=True,
            )
            idled = False
            ...
            for _ in range(60):
                ...
                if idle_state.get("behavior_phase") == "idle":
                    idled = True
                    break
            assert idled, {"detail": "centrifuge never spun down", "state": idle_state}
        finally:
            try:
                cleanup_owned_beamng_session(
                    bng,
    ...
    stage_events = [record for record in records if record["event"] == "centrifuge_stage"]
    assert len(stage_events) >= 4, {"stages": stage_events}
    assert events.index("centrifuge_armed") < events.index("centrifuge_flung")
    assert not issues, issues
    print(
        json.dumps(
            {
                "mod": MOD_ID,
                "stages_reached": len(stage_events),
                "events": events,
            },
            sort_keys=True,
        )
