...
**(2026-08-26: `spin_cycle_washer` re-cut with the bevel applied in order and a
double-cover gate added — see "The washer round" addendum at the end of this
section. `catapult_seesaw` and `giant_toaster` still carry the pending bevel.)**
...
#### The washer round: spin_cycle_washer re-cut (2026-08-26)

The go/no-go this section's blast-radius paragraph deferred was given, and the
washer half of the debt is paid. What landed, each half measured on the SHIPPED
artifact, not the source:

- **The defect this bug shipped on the washer was worse than the fold table
  shows.** The front panel (plane y=+6.5 in the DAE, y=-6.5 authored — the
  exporter negates X and Y) carried **0.083380 m² of SAME-WINDING coplanar
  double cover** (triangles 8139/8140, `enamel_white`): the pending 0.4 bevel,
  applied by the depsgraph to the boolean's OUTPUT, chamfered the bore rim
  (r 4.35, bottom z 0.25) down to z -0.15 — below the machine's own floor —
  where it collided with the box's bottom-edge bevel and emitted a
  self-overlapping fan. `planar_folds`' `sum|a| - |sum a|` invariant is ZERO on
  it by construction (same winding adds to both sums — its docstring's blind
  spot #1). Not tunable around: `use_clamp_overlap` measured zero effect, and
  NARROWING the bevel made it worse (0.25 → 0.210 m²).
- **The fix is `proplib.blender_kit.apply_pending_bevels(body)` before
  `cut_openings`** — apply the bevel in stack order, cut a sharp bore.
  Measured on the re-cut DAE: overlap → 0, front-panel fold → 0, body
  882 → 368 triangles, z-min back to 0.000000, degenerates in the body 0.
  Visual cost, measured: the mouth loses its 0.4 rounded lip (opening
  4.75 → the cutter's true 4.35); `porthole_trim` (major 4.62, tube 0.34,
  inner edge 4.28) covers the rim, and cull-aware A/B renders of shipped vs
  re-cut DAEs from three player angles put the fix's whole visual footprint
  at ≤ 0.06% of pixels. The washer row in the round-4 table above is the
  historical BEFORE measurement; do not reconcile it against the new cut.
- **The gate that sees the class**: `test_no_same_winding_coplanar_double_cover`
  (pack suite) — per index-connected body, plane offsets CLUSTERED with a
  50 µm single-linkage gap (NOT planar_folds' 2 µm offset lattice, its blind
  spot #2: this panel's vertices export at 6.499999/6.5/6.500001 and the
  lattice splits one plane into three buckets, hiding the defect), then
  same-side pairwise polygon clipping. Its synthetic teeth test caught an
  inherited trap on first run: picking the canonical side from the FIRST
  normal component above 1e-9 (planar_folds' rule) lets 1 µm noise choose a
  noise axis as lead and file the two layers in buckets that are never
  compared — the gate picks the LARGEST component. Calibrated over every
  shipped DAE on the branch: the washer was the only instance, zero noise
  findings. (The main checkout's unlanded `colossus_tire` measured 20
  `chock_paint` clusters totalling 0.0288 m²; its landing round will trip
  this gate and should re-cut, not widen the floor.)
- **The re-cut obeyed "dist is a RE-ZIP"**: 57 of 66 members byte-identical
  to the shipped serial-22 zip. Movers, each with a proven cause: the main
  DAE (the fix; all 29,420 non-body triangles multiset-identical, every
  difference inside the 882→368 body); the five part DAEs (positions,
  topology and UVs multiset-IDENTICAL — their normals re-derive from
  today's tracked toolchain, because the 2026-08-15 cut predates source
  tracking (b5f1724, 08-18) and NO tracked toolchain reproduces its corner
  normals; restoring the old bytes is impossible without hand-editing
  handoff hashes); the two thumbnails (re-render of the fixed scene);
  `energy_label.color.png` (catch-up: the shipped bake is an orphan of a
  superseded texture_kit — main, feature tip and this branch all regenerate
  the same new bytes, sha ceb16cc675ed); and **`runtime.lua` RESTORED from
  the snapshot**, because the shipped member was generated by the FEATURE
  branch's newer lua_kit — byte-proven by regenerating it with that lua_kit
  from the POST-FIX handoff (67,253 bytes, identical) — and regenerating it
  with this branch's older lua_kit would have stripped 10.8 KB of shipped
  behavior. Serial 22 → 23, member stamp 2026-08-24T00:00 (below the clamp,
  deterministic). Bonus: the shipped DAEs leaked 20 absolute
  `/C:/Users/<name>/...` texture paths (cut before the path-leak fix landed);
  the re-cut carries 0.
- **Not installed anywhere by this round.** The re-cut zip sits in
  `spin_cycle_washer/dist/` only; the player's installed copy still has the
  membrane and the path leak until Eric installs the new zip (sentinel
  validation first, per the live-gate rules above).
