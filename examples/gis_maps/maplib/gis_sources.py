...
class Footprint:
    ...
    def buffered(self, margin_m: float) -> Footprint:
        return Footprint(
            self.epsg, self.west - margin_m, self.south - margin_m, self.size_m + 2 * margin_m
        )
...
def tile_grid(fp: Footprint, tile_m: float) -> list[tuple[int, int, Tile]]:
    ...
    count = math.ceil(fp.size_m / tile_m)
    tiles = []
    ...
    for ix, iy, tile in tile_grid(fp, tile_m):
        ...
        if not force and path.is_file() and sidecar.is_file() and path.stat().st_size > 0:
            try:
                ...
                pass  # unreadable sidecar: fall through and fetch the tile again
        ...
        if not response.content.startswith(b"\x89PNG"):
            raise RuntimeError(
                f"NAIP export did not return a PNG for {path.name}: {response.text[:200]}"
            )
...
def list_ot_bucket(prefix: str, *, log: Log = _log_default) -> list[tuple[str, int]]:
    ...
    while True:
        ...
        # The bucket listing is public S3 XML from a pinned host; no defusedxml dependency.
        root = ET.fromstring(response.text)  # noqa: S314
        for item in root.findall("s3:Contents", namespace):
            keys.append(
                (item.find("s3:Key", namespace).text, int(item.find("s3:Size", namespace).text))
            )
...
def fetch_ot_grid_window(
    ...
    *,
    ...
    with rasterio.open(grid_dir / open_name) as source:
        ...
        window = (
            from_bounds(*src_bounds, transform=source.transform).round_offsets().round_lengths()
        )
