...
def target_grid(fp, size_px: int, res: float) -> Grid:
    return Grid(
        np.full((size_px, size_px), np.nan, dtype="float32"), fp.epsg, fp.west, fp.north, res
    )
...
def feathered_composite(
    base: np.ndarray, overlay: np.ndarray, feather_px: int = 24
    ...
    out = (
        np.where(np.isfinite(base), base, 0.0) * (1.0 - weight)
        + np.where(valid, overlay, 0.0) * weight
    )
...
def hillshade(
    dem: np.ndarray, res: float, azimuth_deg: float = 315.0, altitude_deg: float = 45.0
) -> np.ndarray:
...
def write_ter(
    path: Path, heights_south_up: np.ndarray, layer_south_up: np.ndarray, materials: list[str]
) -> None:
...
def write_png8(path: Path, array_u8: np.ndarray) -> None:
    ...
    if array_u8.ndim == 2:
        Image.fromarray(array_u8.astype("uint8"), mode="L").save(
            path, format="PNG", compress_level=6
        )
    else:
        Image.fromarray(array_u8.astype("uint8"), mode="RGB").save(
            path, format="PNG", compress_level=6
        )
