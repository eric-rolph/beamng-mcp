...
Usage (from the repository root):
    ...
    python examples/gis_maps/join_parts.py <folder containing the .part files>

For every map whose parts are present it writes
``examples/gis_maps/<key>/dist/<key>_ericrolph.zip`` and
``examples/gis_maps/<key>/dist/ericrolph_<key>.lock.json``. A rejoined ZIP whose SHA-256
does not match ``SHA256SUMS.txt`` is deleted, never installed.
"""
    ...
    for key, parts in sorted(parts_by_key.items()):
        ...
        for part in parts:
            ...
            if expected and digest != expected:
                zip_path.unlink()
                print(
                    f"{key}: rejoined ZIP hash {digest[:16]}... "
                    f"!= expected {expected[:16]}..., deleted"
                )
                failures += 1
                continue
            ...
            note = (
                ...
                if expected
                else "no SHA256SUMS.txt entry; integrity-checked only"
            )
            print(
                f"{key}: {zip_basename} ({lock['size'] / 1e6:.1f} MB, "
                f"{lock['members']} members) {note}"
            )
