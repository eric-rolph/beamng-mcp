...
Usage (from the repository root):
    ...
    python examples/gis_maps/build.py <map_key> fetch    # public data -> <map>/data/ (cached)
    python examples/gis_maps/build.py <map_key> terrain  # data -> heightmap, layer map, stats
    python examples/gis_maps/build.py <map_key> level    # terrain -> mod/levels/<mod_id>/ tree
    python examples/gis_maps/build.py <map_key> dist     # mod/ -> dist ZIP + SHA-256 lock (re-zip)
    python examples/gis_maps/build.py <map_key> ledger   # handoff -> Build ledger in DESIGN.md
    python examples/gis_maps/build.py <map_key> all
    python examples/gis_maps/build.py --all <stage>
...
def load_spec(map_key: str):
    ...
    loader = importlib.util.spec_from_file_location(f"gis_maps_spec_{map_key}", spec_path)
